...
function OverviewScreen({ onOpen }) {
  const statusCounts = { approved: 1, review: 1, needs_info: 1, analyzing: 1, draft: 1, blocked: 1 };
  const statusMeta = [
  ...
  return (
    <div style={{ padding: "24px 28px 40px", maxWidth: 1120, margin: "0 auto" }}>
      <div style={{ marginBottom: 22 }}>
        <h1 style={{ font: "var(--weight-semibold) var(--text-h1)/1.2 var(--font-sans)", color: "var(--text-strong)", margin: 0 }}>{PROJECT.name}</h1>
        <p style={{ font: "14px/1.5 var(--font-sans)", color: "var(--text-muted)", margin: "6px 0 0", display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ font: "12px var(--font-mono)", color: "var(--text-subtle)", direction: "ltr" }}>#{PROJECT.id}</span>
          <span>·</span><span>٦ متطلبات</span><span>·</span><span>٥ أصحاب مصلحة</span>
        </p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
        <Card padding="lg">
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
            <span style={{ font: "var(--weight-semibold) 14px var(--font-sans)", color: "var(--text-strong)" }}>جاهزية المتطلبات للاعتماد</span>
        ...
        <Card padding="lg" style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
          ...
          <div style={{ font: "var(--weight-bold) 32px/1 var(--font-sans)", color: "var(--text-strong)" }}>78<span style={{ fontSize: 18, color: "var(--text-muted)" }}>٪</span></div>
          <div style={{ marginTop: 10 }}><ConfidenceMeter value={78} label="٢٢ من ٢٨ مكتملة" /></div>
        </Card>
        <Card padding="lg" style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <span style={{ font: "12px var(--font-sans)", color: "var(--text-muted)", marginBottom: 8 }}>متوسط ثقة الذكاء الاصطناعي</span>
          <div style={{ font: "var(--weight-bold) 32px/1 var(--font-sans)", color: "var(--teal-600)" }}>74<span style={{ fontSize: 18, color: "var(--text-muted)" }}>٪</span></div>
          <div style={{ marginTop: 10, font: "12px/1.5 var(--font-sans)", color: "var(--text-muted)" }}>متطلب واحد بثقة منخفضة يحتاج مراجعة بشرية.</div>
        </Card>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <Card padding="none">
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "14px 18px", borderBottom: "1px solid var(--border-subtle)" }}>
            ...
            <Badge tone="warning" style={{ marginInlineStart: "auto" }}>{missing.length}</Badge>
          </div>
          <div style={{ display: "flex", flexDirection: "column" }}>
            {missing.map((m, i) => (
              <button key={m.req} onClick={() => onOpen && onOpen(REQUIREMENTS.find((r) => r.id === m.req) || REQUIREMENTS[0])} style={{ display: "flex", gap: 11, alignItems: "flex-start", padding: "13px 18px", borderTop: i ? "1px solid var(--border-subtle)" : "none", background: "transparent", border: "none", borderTopColor: "var(--border-subtle)", cursor: "pointer", textAlign: "start" }}>
        ...
        <Card padding="none">
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "14px 18px", borderBottom: "1px solid var(--border-subtle)" }}>
            ...
            <Button variant="ghost" size="sm" style={{ marginInlineStart: "auto" }} onClick={() => onOpen && onOpen(null)}>عرض الكل</Button>
          </div>
          <div style={{ display: "flex", flexDirection: "column" }}>
            {REQUIREMENTS.slice(0, 4).map((r, i) => (
              <button key={r.id} onClick={() => onOpen && onOpen(r)} style={{ display: "flex", gap: 11, alignItems: "center", padding: "12px 18px", borderTop: i ? "1px solid var(--border-subtle)" : "none", background: "transparent", border: "none", cursor: "pointer", textAlign: "start" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ font: "var(--weight-medium) 13px/1.4 var(--font-sans)", color: "var(--text-strong)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.title}</div>
                </div>
