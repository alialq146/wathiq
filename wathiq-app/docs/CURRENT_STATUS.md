...
| البند | الحالة |
|------|--------|
| الإصدار | **2.6.1** (`src/lib/version.ts`) |
...
| الاختبارات الآلية | سكربتات QA على Postgres حقيقي (لا CI بعد): محاسبة الذكاء **93 فحصًا** (`qa-credits/reaper/operation/settings/billing/http`) + regression (`qa-v23/v24`) + tsc + build. `npm run lint` غير مُهيّأ في المستودع (`next lint` يطلب إعدادًا أوليًا). انظر `docs/TESTING.md` |
