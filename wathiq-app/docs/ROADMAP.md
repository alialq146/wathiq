...
1. **تاريخ التحليل يُفقد اليوم (#4)**: `Requirement.analysis Json?` عمود مفرد يُكتب فوقه كل إعادة تحليل، و`AiUsage` يسجّل المحاولة لا النتيجة. إن كانت الجلسات/التاريخ محتملة، فالتحضير الأرخص = التوقّف عن الكتابة فوقه (إلحاق بمصفوفة أو جدول `AnalysisSession`) **قبل فقدان مزيد من التاريخ**. هذا البند الوحيد الذي للتأجيل فيه كلفة مستمرة.
