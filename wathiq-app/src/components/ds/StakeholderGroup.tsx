...
export type Stakeholder = string | { name: string; src?: string };

export interface StakeholderGroupProps
...
export function StakeholderGroup({
  people = [],
  max = 4,
  size = 28,
  label,
  ...
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 10, ...style }} {...rest}>
      ...
      {label && (
        <span style={{ font: "var(--weight-regular) var(--text-sm)/1 var(--font-sans)", color: "var(--text-muted)" }}>
          {label}
        </span>
      )}
    </div>
